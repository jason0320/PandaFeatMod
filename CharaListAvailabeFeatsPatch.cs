...
class CharaListAvailabeFeatsPatch
{
    ...
    internal static void Postfix(bool pet, ref List<Element> __result)
    {
        foreach (SourceElement.Row item in EClass.sources.elements.rows.Where((SourceElement.Row a) => a.group == "FEAT" && a.cost[0] != -1 && !a.categorySub.IsEmpty()))
        {
            ...
            if (num <= feat.source.max && (feat.HasTag("class") || feat.HasTag("hidden") || feat.HasTag("innate")) && (!pet || !feat.HasTag("noPet")) && feat.IsAvailable(Act.CC.elements, feat.Value))
            {
